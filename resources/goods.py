...
class GoodsSearchResource(Resource) :
    # 검색한 결과 가져오기
    def get(self) :
        offset = request.args.get('offset')
        limit = request.args.get('limit')   
        ...
        try :
            ...
            if int(category) == 0:
                ...
                query = '''select g.* , wishCount.wishCount, commentCount.commentCount, imgCount.imgCount
                            ...
                            join tag_name tn
                            on t.tagNameId = tn.id
                            where tn.name like '%{}%' or g.title like '%{}%' or g.content like '%{}%'
                            group by g.id) g,
                            (select g.id, count(wl.id) wishCount 
                            from goods g
                            left join wish_lists wl
                            ...
                            where g.id = wishCount.id and g.id = commentCount.id and g.id = imgCount.id
                            order by g.createdAt desc
                            limit {}, {};'''.format(keyword, keyword, keyword,  offset, limit)
                print('done')
            else :
                # 게시글 가져오기
                # imageCount : 이미지 등록수, wishCount : 관심 등록 수, commentCount : 댓글 등록수
                query = '''select g.* , wishCount.wishCount, commentCount.commentCount, imgCount.imgCount
                            ...
                            where g.id = wishCount.id and g.id = commentCount.id and g.id = imgCount.id and g.categoriId = {}
                            order by g.createdAt desc
                            limit {}, {};'''.format(keyword, keyword, keyword, category, offset, limit)
            # 3. 커서를 가져온다.
            # select를 할 때는 dictionary = True로 설정한다.
            cursor = connection.cursor(dictionary = True)
...
class LoginStatusGoodsSearchResource(Resource) :
    # 로그인 상태일 때 검색한 결과 가져오기
    @jwt_required()
    def get(self) :
        offset = request.args.get('offset')
        limit = request.args.get('limit')   
